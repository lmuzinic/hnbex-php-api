...
class BackendTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function setUp()
    {
        ...
        $this->backend = new Backend($this->client, $cachePool);
    }
    ...
    public function testSendReturnsErrorWhenApiHasProblems()
    {
        $this->response->method('getStatusCode')->willReturn(503);
        $this->stream->method('getContents')
            ->willReturn('{"error": "Can\'t connect to HNB server"}');
        $response = $this->backend->send(OnDate::fromString('today'));

        $this->assertInstanceOf(Response::class, $response);
        $this->assertInstanceOf(Error::class, $response);
        $this->assertInstanceOf(\Hnbex\Common\Error::class, $response->getContent());
        $this->assertFalse($response->isCached());
    }

    public function testSendReturnsCachedCurrencyExchangeRateCollection()
    {
        $cachePool = $this->createMock(VoidCachePool::class);
        $cachePool->method('has')->willReturn(true);
        $cachePool->method('get')->willReturn(new CurrencyExchangeRateCollection());
